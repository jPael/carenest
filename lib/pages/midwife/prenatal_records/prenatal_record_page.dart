...
import 'package:smartguide_app/models/after_care.dart' as afModel;
...
class _PrenatalRecordPageState extends State<PrenatalRecordPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    tabViews = [
      ...
      const BirthPlan(),
      AfterCare(
        afterCare: afModel.AfterCare(
            immunzation: widget.prenatal.ttItems.map(
              (e) {
                return afModel.Immunization(term: e['term']);
              },
            ).toList(),
            ironSupplement: widget.prenatal.ironSuppItems
                .map(
                  (e) => afModel.IronSupplement(tabs: e['no_tabs']),
                )
                .toList()),
      ),
      const Counseling()
