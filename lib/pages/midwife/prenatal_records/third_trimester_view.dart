...
import 'package:smartguide_app/components/midwife/prenatal_records/clinic_view.dart/third_trimester_check_up_selector_dropdown.dart';
...
class ThirdTrimesterView extends StatefulWidget {
  const ThirdTrimesterView({super.key, required this.trimesters, required this.defaultSelected});

  final List<ThirdTrimester> trimesters;
  final int defaultSelected;

  @override
  ThirdTrimesterViewState createState() => ThirdTrimesterViewState();
}

class ThirdTrimesterViewState extends State<ThirdTrimesterView> {
  late ThirdTrimester selected;
    ...
    return Scaffold(
        appBar: AppBar(
          title: Column(
            ...
            children: [
              ThirdTrimesterCheckUpSelectorDropdown(
                onChange: (ThirdTrimester? value) {
                  setState(() {
        ...
        body: SingleChildScrollView(
          child: Padding(
            ...
            child: CustomSection(
                title:
                    "Clinic Date: ${DateFormat("EEEE MMMM, dd yyyy").format(selected.createdAt)}",
                ...
                children: [
                  ...
                  ItemGroup(
                      title: "Advices", data: selected.advices.map((a) => a.content).toList()),
                  TestItem(
                  ...
                  TestItem(
                    ...
                    value: selected.bloodRhGroup,
                  ),
                  ItemGroup(
                    title: "Treatments",
                    data: selected.treatments.map((a) => a.description).toList(),
                    // description: selected.treatments.map((a) => a.description).toList(),
                  ),
                  ItemGroup(
